...
def add_submit(
    ...
    for idx, v in enumerate(vids, start=len(pics) + 1):
        ...
        # ── Vídeo NATIVO do ML (HLS / .m3u8) ──────────────────────────────
        if mtype == "video" and downloader.is_hls_url(vid_url):
            m = Media(
            ...
            try:
                meta = downloader.download_ml_video(vid_url, dest, idx)
                m.local_path = meta["local_path"]
                m.file_size = meta["file_size"]
                m.width = meta["width"]
                m.height = meta["height"]
                m.downloaded_at = datetime.utcnow()
                logger.info("Vídeo nativo ML salvo: %s", m.local_path)
            except Exception as e:
                logger.error("Falha ao baixar vídeo ML %s — %s", vid_url[:80], e)
                m.download_error = str(e)
            db.add(m)

        # ── YouTube: salva URL + baixa thumbnail ──────────────────────────
        elif mtype == "youtube" and v.get("id"):
            m = Media(
                listing_id=listing.id,
                type="youtube",
                source_url=vid_url,
        ...
        # ── Outros tipos de vídeo: só salva a URL fonte ───────────────────
        else:
            m = Media(
                listing_id=listing.id,
                type="video",
